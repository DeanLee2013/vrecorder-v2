...
| 3 | AirPods/Bluetooth mic unavailable + route-add ignored | `AudioSessionController` | Medium | TODO | From bug-#1 Gate-4 audit. `.playAndRecord` omits `.allowBluetoothHFP`; only `.oldDeviceUnavailable` handled. Fix: add BT option + handle `.newDeviceAvailable` (reconfigure/restart). GH: #5. |
