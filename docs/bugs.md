...
| 8 | pre-push hook false-matches prompt 'VERDICT: BLOCK' → blocks genuine PASS | `scripts/git-hooks/pre-push` | High | FIXED | Unanchored grep matched the prompt's instruction text. Fixed (anchored `^[[:space:]]*VERDICT: BLOCK[[:space:]]*$`). Fixed hook now **installed** (`install.sh`) and proven: the v0.6.0 push (e646a2a) is the first genuine `VERDICT: PASS` to flow through the gate without `--no-verify`; old hook false-blocked the same commit. Evidence `bug-8-20260615.md` (verification-exception: git-hook, not device-reproducible). GH: see issue. |
| 9 | Speech failures all map to recognitionFailed (no perm/network distinction) | `AppleSpeechRecognizer` | Low | FIXED | From feature-#6 WI-3 audit. Recheck authorization + map connectivity errors before the generic recognitionFailed fallback. GH: #24. |
