...
| 2 | Unbounded partial-result stream + per-callback Task accumulation | `AppleSpeechRecognizer` | Medium | FIXED | Same audit; long live sessions can accumulate partials/tasks. Fix: bounded event pump that coalesces partials, preserves finals. GH: #4. |
